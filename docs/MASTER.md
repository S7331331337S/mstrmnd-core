...
- Stronger per-tool `evaluateBoundaryAction` on every dispatch (attach on createRun is landed)
...
Next hardening: broader harness adapters / plugin SDK after this Operator Zero stack. Policy-gated workspace writes, model-proposed parent planning, `mstrmnd-os` Node 24 typecheck in CI, and fail-closed ThreatBoundary attach landed. `openai` / `openai-compatible` already landed; CI still defaults to `echo`.
...
- **Code maturity:** Operator Zero runtime with context pack, workspace mounts, policy-gated writes (draft → approve → publish), Hermes orchestrator, MCP plugin tools, operator-pack template, openai-compatible provider (CI default `echo`), fail-closed ThreatBoundary on createRun; Board decision-room imported at `apps/board`
