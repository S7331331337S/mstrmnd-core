...
export function assertBoundary(
  ...
  if (
    !Number.isFinite(boundary.costCeilingUsd) ||
...
function pathInScope(
  ...
  const parts = path
    ...
    .split("/")
    .filter(Boolean);
  // Reject traversal before prefix matching; the workspace resolves dot segments.
...
export function evaluateBoundaryAction(
  ...
  if (!Number.isFinite(accrued) || !Number.isFinite(estimated) || accrued < 0 || estimated < 0 || accrued + estimated > bound.costCeilingUsd) {
    return deny(
      action.toolId,
