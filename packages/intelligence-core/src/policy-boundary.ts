...
export function assertBoundary(
  ...
  if (!boundary.id?.trim() || !boundary.workflowId?.trim()) {
    throw new MissingBoundaryError("threat boundary id and workflowId are required");
  }
  ...
  if (typeof boundary.costCeilingUsd !== "number" || boundary.costCeilingUsd < 0) {
    throw new MissingBoundaryError("threat boundary costCeilingUsd must be >= 0");
  }
}
...
function hostAllowed(allowlist: string[], destination: string): boolean {
  ...
  return allowlist.some((entry) => {
    ...
    if (dest === allowed || dest.endsWith("." + allowed)) return true;
    if (dest.startsWith(allowed)) return true;
    return false;
  });
}
...
function pathInScope(
  ...
  return scope.some((entry) => {
    ...
    const prefix = entry.pathPrefix.replace(/\\/g, "/").replace(/^\/+/, "");
    if (!prefix) return true;
    return normalized === prefix || normalized.startsWith(prefix.replace(/\/+$/, "") + "/");
  });
}
...
export function evaluateBoundaryAction(
  ...
  try {
    ...
    return deny(action.toolId, err instanceof Error ? err.message : "missing boundary", "missing-boundary");
  }
  const bound: ThreatBoundary = boundary;

  const writeLike = /write|delete|publish|stage|send|patch|merge/i.test(action.toolId);
  const consequential =
  ...
  if (!bound.toolsAllowlist.includes(action.toolId)) {
    return deny(action.toolId, `tool ${action.toolId} is not on the boundary allow-list`, "tool-not-allowed");
  }
