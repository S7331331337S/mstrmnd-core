...
test("filesystem boundary rejects traversal through an allowed prefix", () => {
  for (const path of ["20-Areas/../secret.md", "20-Areas/./secret.md", "20-Areas\\..\\secret.md"]) {
    assert.equal(evaluateBoundaryAction(base, {
      toolId: "read_file", filesystem: { mountId: "vault", path },
    }).outcome, "deny");
  }
});

test("non-finite ceilings and invalid cost accounting fail closed", () => {
  for (const cost of [NaN, Infinity, -Infinity]) {
    assert.throws(() => assertBoundary({ ...base, costCeilingUsd: cost }), MissingBoundaryError);
  }
  for (const cost of [NaN, Infinity, -1]) {
    assert.equal(evaluateBoundaryAction(base, { toolId: "search_memory", accruedCostUsd: cost }).outcome, "deny");
    assert.equal(evaluateBoundaryAction(base, { toolId: "search_memory", estimatedCostUsd: cost }).outcome, "deny");
  }
});
